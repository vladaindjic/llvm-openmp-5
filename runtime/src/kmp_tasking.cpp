...
static void __kmpc_omp_task_begin_if0_template(ident_t *loc_ref, kmp_int32 gtid,
  ...
  if (ompt) {
    if (current_task->ompt_task_info.frame.enter_frame.ptr == NULL) {
      // FIXME vi3: suppose we can get frame_address here?
...
static kmp_int32 __kmpc_omp_taskwait_template(ident_t *loc_ref, kmp_int32 gtid,
  ...
  if (__kmp_tasking_mode != tskm_immediate_exec) {
    ...
    if (ompt) {
      ...
      // FIXME VI3: Do we really need to provide frame adrrese, or it can
      //   found here?
...
kmp_int32 __kmpc_omp_taskwait(ident_t *loc_ref, kmp_int32 gtid) {
  ...
  if (UNLIKELY(ompt_enabled.enabled)) {
    OMPT_STORE_RETURN_ADDRESS(gtid);
    return __kmpc_omp_taskwait_template<true>(loc_ref, gtid, OMPT_GET_FRAME_ADDRESS(0),
                                              NULL);
